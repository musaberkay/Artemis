...
For more details, see the [Auxiliary Repositories configuration section](/instructor/exercises/programming-exercise#auxiliary-repositories-details) in Exercise Creation.
...
| No. | Language   | [Sequential Test Runs](#sequential-test-runs) | [Static Code Analysis](#configure-static-code-analysis) | [Plagiarism Check](../assessment-grading/plagiarism-check) | [Package Name](#package-name) | Project Type           | [Solution Repository Checkout](#solution-repository-checkout) | [Auxiliary Repositories](#auxiliary-repositories) |
|:---:|:-----------|:---------------------------------------------:|:-------------------------------------------------------:|:---------------------------------------:|:-----------------------------:|:-----------------------|:-------------------------------------------------------------:|:-------------------------------------------------:|
...
3. [Adapt build phases (optional)](#adapt-build-phases-optional)
...
- **Build plan**: A default build plan is created for your exercise. You can customize it after creation on the dedicated build plan editor page, see [Adapt Build Phases](#adapt-build-phases-optional). Available for all languages with Local CI and Jenkins
...
- **Online Code Editor** (<img src={editInEditor} alt="Edit in Editor" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} />) — live synchronization of code files, the file tree, and the problem statement.
- **Edit Page** (<img src={edit} alt="Edit" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} />) — synchronization of exercise settings and the problem statement.
...
When someone pushes a commit to a repository (e.g., via Git CLI, an IDE, or "Submit" button) while the online code editor is open, a **new commit alert** is displayed. This advises you to save your work by clicking <img src={submitButton} alt="Submit" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} /> and refresh the page to incorporate the new changes. The alert only appears for the repository that received the new commit.
...
**Problem Statement Synchronization:** The markdown problem statement editor supports the same real-time collaboration with cursor awareness. This works both in the online code editor (<img src={editInEditor} alt="Edit in Editor" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} />) and on the edit page (<img src={edit} alt="Edit" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} />).
...
**Exercise Settings Synchronization:** On the edit page (<img src={edit} alt="Edit" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} />), changes to exercise settings (title, points, dates, grading criteria, etc.) are synchronized to other users when saved. Non-conflicting changes are applied automatically. If both instructors modified the same fields, a merge conflict modal lets you choose which values to keep.
...
<Callout variant={CalloutVariant.info}>
    ...
    <ul>
        <li>Click <img src={submitButton} alt="Submit" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} /> frequently to ensure your work is committed to the repository.</li>
...
All AI-generated modifications — from generation, refinement, quality fixes, and difficulty adaptation — are presented in a diff view before application. The left column shows the current problem statement with deletions highlighted in red, while the right column shows the proposed revision with additions highlighted in green. A summary bar reports the net change statistics. You can accept, discard, or toggle between split and inline views.
...
Only **Java** and **Kotlin** exercises are affected, because their builds resolve dependencies via Maven Central. There is currently no central Artemis-side solution, so instructors must configure the mirror in each exercise's **test repository**.
...
A new exercise starts with a default build plan for its programming language. To adapt it, open the exercise's detail page and click **Edit build plan**. All changes apply to all builds (template, solution, and student submissions).
The build plan editor shows the template and solution build status next to the configuration, so you can immediately see whether your changes still build. You can also view the configured build plan on the exercise's detail page.
...
You edit this on the dedicated build plan editor page, which you open with the **Edit build plan** button on the exercise detail page (see [Adapt Build Phases](#adapt-build-phases-optional)).
...
Click <img src={edit} alt="Edit" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} /> on the programming exercise or navigate to <img src={editInEditor} alt="Edit in Editor" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} /> and adapt the interactive problem statement. When multiple instructors edit the problem statement simultaneously, changes are synchronized in real time. See <a href="#collaborative-editing">Collaborative Editing</a> for details on real-time collaboration features.
...
<Callout variant={CalloutVariant.info}>
  ...
  The GCC SCA option for C does not offer categories by default. The issues were categorized during parsing with respect to the rules. For details on the default configuration and active rules, see the <a href="#c">GCC - Static Code Analysis Default Configuration</a> section below.
